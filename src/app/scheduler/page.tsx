            ...
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Button asChild variant="secondary" size="sm">
                  <Link href="/api/export/pdf"><FileDown className="h-4 w-4" />PDF</Link>
                </Button>
                <Button asChild variant="secondary" size="sm">
                  <Link href="/api/export/excel"><FileSpreadsheet className="h-4 w-4" />Excel</Link>
                </Button>
            ...
            <CardContent>
              <div className="grid gap-3 md:grid-cols-3">
                {["Horario docente", "Horario curso", "Horario aula"].map((view) => (
                  <button key={`scheduler-view-${view}`} className="rounded-2xl border bg-secondary px-4 py-3 text-sm font-semibold transition hover:bg-secondary/70">
                    {view}
                  </button>
                ))}
              </div>
