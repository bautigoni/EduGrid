...
import { getCourseAvailability, saveCourseAvailability } from "@/server/repositories/availability";

export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    await requireAuth();
    const { id } = await params;
    return NextResponse.json(getCourseAvailability(id));
  } catch (error) {
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    saveCourseAvailability(id, body.entries);
