    ...
    <AppShell title="Proyectos, electivas y optativas" subtitle="Coordiná bloques especiales con docentes y cursos simultáneos.">
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        <div className="flex justify-end">
          <Button><Plus className="h-4 w-4" />Crear proyecto/electiva</Button>
        </div>
        <div className="grid gap-4 xl:grid-cols-2">
          {data.programBlocks.map((block) => (
            <Card key={block.id} className={block.priority === "CRITICAL" ? "border-orange-300" : ""}>
              <CardHeader>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Badge>{typeLabels[block.type] ?? block.type}</Badge>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border p-3">
                    ...
                    <div className="flex flex-wrap gap-2">{block.requiredTeachers.map((teacher) => <Badge key={`${block.id}-teacher-${teacher}`}>{teacher}</Badge>)}</div>
                  </div>
                  <div className="rounded-2xl border p-3">
                    <p className="mb-2 text-sm font-semibold">Cursos involucrados</p>
                    <div className="flex flex-wrap gap-2">{block.involvedCourses.map((course) => <Badge key={`${block.id}-course-${course}`}>{course}</Badge>)}</div>
                  </div>
                </div>
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <div className="rounded-xl bg-secondary p-3 text-sm">{block.weeklyBlocksRequired} hs semanales</div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {block.requiresSameTimeTeachers && <Badge className="bg-orange-500/10 text-orange-700">Docentes simultáneos</Badge>}
                  {block.requiresSameTimeCourses && <Badge className="bg-green-500/10 text-green-700">Cursos simultáneos</Badge>}
                  {block.fixedDay !== null && <Badge><Clock className="mr-1 h-3 w-3" /> Fijo: día {block.fixedDay} · bloque {block.fixedBlockIndex}</Badge>}
                </div>
